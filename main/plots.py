...
from source.LCF import (compute_normalized_auc, estimate_lcf,
                        estimate_point_counts, plot_lcf, test_LCF)
from source.simulation import (DummyModel, random_gaussian_mixture_func,
                               random_GMM_samples, random_GP_func,
                               random_linear_func, random_matern_samples,
                               random_thomas_samples, random_uniform_samples)
...
def plot_cov_KL_estim(conf, params, KL_estim_list=["naive", "scipy", "skl", "skl_ef"]):
    ...
    os.makedirs(
        f"./main/results/KL_plots/{params['model']}_{params['f']}", exist_ok=True
    )
...
def plot_cov_KS(conf, params, n_tests=5):
    ...
    for j, cov in tqdm(
        enumerate(params["max_cov_list"]), total=len(params["max_cov_list"])
    ):
        conf["max_cov"] = cov
        g_gen = partial(samples[params["samples"]], conf)
        p_gen = partial(random_uniform_samples, conf, True)
        ...
        for i in trange(n_tests, desc=f"Tests for cov={cov}"):
            ...
            ks_stat_g_lat, _ = kstest(
                lat_g, "uniform", args=(np.min(lat_g), np.ptp(lat_g))
            )
            ks_stat_g_long, _ = kstest(
                long_g, "uniform", args=(np.min(long_g), np.ptp(long_g))
            )
            ks_stat_p_lat, _ = kstest(
                lat_p, "uniform", args=(np.min(lat_p), np.ptp(lat_p))
            )
            ks_stat_p_long, _ = kstest(
                long_p, "uniform", args=(np.min(long_p), np.ptp(long_p))
            )
            ...
            cumulative_ks_2d_p += energy_distance_2d(
                p_sample, np.random.uniform(size=(len(p_sample), 2)) * 100
            )
            cumulative_ks_2d_g += energy_distance_2d(
                g_sample, np.random.uniform(size=(len(g_sample), 2)) * 100
            )
...
def plot_cov_LCF(conf, params, n_tests=3):
    rmax = 0.188 * conf["max_mu"] * 7  # 5

    r_values = np.arange(0, rmax, 1)
    test_LCF()
    samples = {
        "GMM": random_GMM_samples,
        "Thomas": random_thomas_samples,
    ...
    for j, cov in tqdm(enumerate(params["max_cov_list"])):
        ...
        if params["p_style"] == "uniform":
            ...
            conf_p["max_cov"] = 400
            p_gen = partial(samples["GMM"], conf_p)
        ...
        for i in trange(n_tests):
            ...
            AUC_p = compute_normalized_auc(
                p_lcf_df, r_values, rmin=rmax / 10, rmax=rmax
            )
            AUC_g = compute_normalized_auc(
                g_lcf_df, r_values, rmin=rmax / 10, rmax=rmax
            )
    ...
    log.info(params["p_style"])
    log.info(LCF_dict)
    log.info(params["max_cov_list"])
    return True
