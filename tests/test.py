...
def test(
    ...
    n_splits=1,
    target_error=None,
    hyperparams={"kde_size": ["silverman"], "epsilon_reg": [0], "epsilon_clip": [0]},
):
    ...
    for _ in trange(n_tests):
        ...
        for i, (train_idx, test_idx) in enumerate(kf.split(g_sample)):
            ...
            if [i for i in target_error if i in estim_list]:
                kde = KernelDensity(
                    kernel="gaussian", bandwidth=hyperparams["kde_size"][0]
                ).fit(g_train)
            ...
            if "ISE_g_regular" in target_error:
                ...
                for epsilon in hyperparams["epsilon_reg"]:
                    g_estim_new = lambda X: np.log(
                        (1 - epsilon) * np.exp(g_estim(X))
                        + epsilon / (conf["max_mu"] ** 2)
                    )
                    ...
                    if epsilon == 0:
                        print(
                            f"Check limit for IS on epsilon = 0, delta = {error - importance_sampling_error(err, p, g_estim, g_test)}"
                        )
                ...
                if target_error == ["ISE_g_regular", "MCE_p"]:
                    fig, ax = plt.subplots(
                        figsize=(12, 12), label=f"max_cov = {conf['max_cov']}"
                    )
            ...
            if "ISE_g_clip" in target_error:
                ...
                if target_error == ["ISE_g_clip", "ISE_g_estim_clip", "MCE_p"]:
                    fig, ax = plt.subplots(
                        figsize=(12, 12), label=f"max_cov = {conf['max_cov']}"
                    )
                    ax.plot(hyperparams["epsilon_clip"], errors_plot)
                    plt.title(f"max_cov = {conf['max_cov']}")
                    ax.set_xlabel("epsilon", fontsize=26)
