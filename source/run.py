...
def run_test_case(f_gen, model, g_gen, p_gen, n_tests, n_splits, x, y,
                  save_fig = False,
                  show_fig = False,
    ...
    log_err = test(f_gen=f_gen, model = model, g_gen = g_gen, p_gen = p_gen, n_tests = n_tests, n_splits = n_splits,
...
def run(conf, f, model, n_splits, x, y,
        hyperparams = {'kde_size' : ['scott'],
                        'epsilon_reg' : [0],
                        'epsilon_clip' : [0]}):
    ...
    return run_test_case(f_gen = f_gens[f],
                  model = models[model],
