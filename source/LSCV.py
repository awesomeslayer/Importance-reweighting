...
def squared_error_sklearn(h, conf, g_sample, p_sample, beta):
    ...
    cv = -uniform_sum
    #log.debug(
    #    f"integral = {integral}, subsample = {sub}, beta*uniform_sum = {uniform_sum}, cv = {cv}"
    #)
    log.debug(f"cv = {cv}")
    return cv
...
def KL_LSCV_find_bw(conf, g_sample, p_sample, beta=0):
    ...
    h = minimize(squared_error_sklearn, h0, args=(conf, g_sample, p_sample, beta), constraints=cons).x
