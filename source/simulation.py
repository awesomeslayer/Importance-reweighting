...
def _GMM_gen(max_mu, max_cov, n_components, n_dim):
    ...
    var = max_cov * np.array([random_covariance_matrix(n_dim) for _ in range(n_components)], np.float32)
...
def random_GMM_samples(config):
    ...
    """
    gmm = _GMM_gen(config['max_mu'], config['max_cov'], config['n_components'], config['n_dim'])
