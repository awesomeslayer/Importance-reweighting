...
def errors_test(
    ...
    for train_idx, test_idx in kf.split(g_sample):
        ...
        fill_gen_dict(gen_dict, params,hyperparams_dict, g_sample, p_sample, train_idx, test_idx, log)
        fill_errors(error, error_hyp, conf, hyperparams_dict, params, hyperparams_params, sizes, gen_dict, log)

    log.debug(f"\nfinal error (summed for each fold) for one test num_{i}:\n {error}\n")
    log.debug(f"\nfinal hyperparams error (summed for each fold) for one test num_{i}:\n {error_hyp}\n")

    fill_dicts(err_dict, err_hyp_dict, error, error_hyp, params, hyperparams_params, sizes, log)

    return True

def fill_gen_dict(gen_dict, params, hyperparams_dict, g_sample, p_sample, train_idx, test_idx, log):
    ...
    gen_dict["err"] = lambda X: np.log(np.abs(
        gen_dict["f"](X) - params["model_gen"].predict(X)
    ))
...
def fill_errors(error, error_hyp, conf, hyperparams_dict, params, hyperparams_params, sizes, gen_dict, log):
    ...
    if hyperparams_params["grid_flag"]:
        for j in trange(sizes["max_size"]):
            ...
            for x_temp in x_estim_temp:
                error_hyp[x_temp][j] = error_hyp[x_temp][j] + np.exp(error_hyp_temp[x_temp])
...
def fill_dicts(err_dict, err_hyp_dict, error, error_hyp, params, hyperparams_params, sizes, log):
    for x_temp in params["x"] + params["y"]:
    ...
    if hyperparams_params["grid_flag"]:
        for x_temp in params["x_hyp"]:
            for j in range(sizes["max_size"]):
                if j < sizes[x_temp]:
                    err_hyp_dict[x_temp][j] += [error_hyp[x_temp][j] / params["n_splits"]]
